...
parser.add_argument('-k', help='Insecure connections', action='store_false')
parser.add_argument('--service-name', type=str, help='Service base name', default="fakeservice")
...
for i in range(n_services):
    ...
    req = requests.post(
        ...
        data={'access_token': access_token, 'unit': 'hits', 'friendly_name': service_name + '_hits'}, verify=args.k)
    ret = json.loads(req.text)
    ...
    #Get account ids
    req = requests.get(base_url + "/admin/api/accounts.json", data = {"access_token": access_token}, verify=args.k)
    ret = json.loads(req.text)
    account_ids = []
    ...
    for j in range(n_applications):
        req = req = requests.post(
            ...
            data={'access_token': access_token, 'plan_id': random.choice(plan_ids), 'name': "{0}_app_{1:03}".format(service_name, j),
